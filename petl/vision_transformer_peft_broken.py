...
class MultiLoRALinear(nn.Linear, PEFTLayer):
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
                 r: int = 0, alpha: int = 1, dropout: float = 0., num_loras: int = 2):
        nn.Linear.__init__(self, in_features, out_features, bias=bias)
        PEFTLayer.__init__(self, r=r, alpha=alpha, dropout=dropout)
        ...
        if r > 0:
            ...
            self.lora_A = nn.ParameterList([
                nn.Parameter(torch.zeros(r, in_features)) 
    ...
    def forward(self, x: torch.Tensor, lora_index: int = 0):
        if self.r > 0:
            result = super().forward(x)
            lora_output = (self.dropout(x) @ 
                         self.lora_A[lora_index].T @ 
                         self.lora_B[lora_index].T) * self.scaling
...
class MultiLoRAAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 r=4, alpha=16, num_loras=2):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
        self.scale = self.head_dim ** -0.5

        # 使用MultiLoRALinear替换普通Linear层
        self.q_proj = MultiLoRALinear(dim, dim, bias=qkv_bias, r=r, alpha=alpha, num_loras=num_loras)
        self.k_proj = MultiLoRALinear(dim, dim, bias=qkv_bias, r=r, alpha=alpha, num_loras=num_loras)
        self.v_proj = MultiLoRALinear(dim, dim, bias=qkv_bias, r=r, alpha=alpha, num_loras=num_loras)
        
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = MultiLoRALinear(dim, dim, r=r, alpha=alpha, num_loras=num_loras)
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(self, x: torch.Tensor, lora_index: int = 0):
        ...
        # 使用指定的LoRA进行投影
        q = self.q_proj(x, lora_index)
        k = self.k_proj(x, lora_index)
        v = self.v_proj(x, lora_index)
        ...
        # 使用指定的LoRA进行输出投影
        x = self.proj(attn_output, lora_index)
        x = self.proj_drop(x)
...
class MultiLoRABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., 
                 ...
                 norm_layer=nn.LayerNorm, r=4, alpha=16, num_loras=2):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = MultiLoRAAttention(
        ...
        # MLP with MultiLoRALinear
        self.fc1 = MultiLoRALinear(dim, mlp_hidden_dim, r=r, alpha=alpha, num_loras=num_loras)
        self.act = act_layer()
        self.fc2 = MultiLoRALinear(mlp_hidden_dim, dim, r=r, alpha=alpha, num_loras=num_loras)
        self.drop = nn.Dropout(drop)

    def forward(self, x: torch.Tensor, lora_index: int = 0):
        x = x + self.drop_path(self.attn(self.norm1(x), lora_index))
        residual = x
        x = self.norm2(x)
        x = self.fc1(x, lora_index)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x, lora_index)
        x = self.drop(x)
        x = residual + self.drop_path(x)
        return x
...
class MultiLoRAVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True,
                 representation_size=None, distilled=False, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed,
                 norm_layer=None, act_layer=None, r=4, alpha=16, num_loras=2):
        super().__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim
        self.num_tokens = 2 if distilled else 1
        ...
        # 生成不同深度的drop path rate
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]
        
        # 创建transformer blocks
    ...
    def forward_features(self, x, lora_index: int = 0):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        #     x = torch.cat((cls_tokens, x), dim=1)
        # else:
        #     x = torch.cat((cls_tokens, self.dist_token.expand(B, -1, -1), x), dim=1)
            
        x = x + self.pos_embed
        x = self.pos_drop(x)

        # 通过所有Block，传入lora_index
    ...
    def forward(self, x, lora_index: int = 0):
        x = self.forward_features(x, lora_index)
        x = self.head(x)
        return x
...
def vit_base_patch16_224_in21k_multilora(pretrained=False, num_loras=2, **kwargs):
    # 创建多LoRA模型实例
    model = MultiLoRAVisionTransformer(
        patch_size=16, 
        embed_dim=768, 
        depth=12, 
        num_heads=12, 
    ...
    if pretrained:
        ...
        # 修改checkpoint的state dict以匹配模型结构
        # 1. 拆分qkv权重为q, k, v
        for key in list(state_dict.keys()):
            if 'qkv.weight' in key:
                qkv_weight = state_dict.pop(key)
                ...
                k_weight = qkv_weight[768:768*2]
                v_weight = qkv_weight[768*2:]
                # 由于现在是MultiLoRALinear，权重名称需要相应调整
                state_dict[key.replace('qkv.weight', 'q_proj.weight')] = q_weight
                state_dict[key.replace('qkv.weight', 'k_proj.weight')] = k_weight
                state_dict[key.replace('qkv.weight', 'v_proj.weight')] = v_weight
            elif 'qkv.bias' in key:
